...
export const HomePage: React.FC = () => {
  ...
  const [bestSellers, setBestSellers] = useState<ProductCardResponse[]>([]);
  const [bestSellerIndex, setBestSellerIndex] = useState(0);
  const [bestSellerLoading, setBestSellerLoading] = useState(true);
  const apiCall = useCallback(() => brandApi.getBrandSummaries(), []);
  const { data: brands } = useApi(apiCall, MOCK_BRANDS);
  ...
  useEffect(() => {
    ...
    const loadBestSellers = async () => {
      ...
      try {
        const response = await productApi.getBestSellingProducts({ page: 0, size: 10 });
        const content = (response as { content?: ProductCardResponse[] }).content ?? [];
        if (isMounted) {
          setBestSellers(content);
        }
      } catch {
        if (isMounted) {
          setBestSellers([]);
